...
Item {
  ...
  // Rate limiting for IPC calls: max 30 calls per 10 seconds per target
  property var ipcCallTimes: ({})
  function checkRateLimit(target) {
    var now = Date.now()
    var times = ipcCallTimes[target] || []
    times = times.filter(function(t) { return now - t < 10000 })
    if (times.length >= 30) return false
    times.push(now)
    ipcCallTimes[target] = times
    return true
  }
  ...
  function handleNotification(notification) {
    ...
    // Cap total notifications to prevent DoS
    if (toasts.count >= maxNotifications) {
      var oldestKey = toasts.get(toasts.count - 1).key
      closeToast(oldestKey, "overflow")
    }
  ...
  //
  // So omaping answers to it as well. The names and return values are the
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "omaping"
